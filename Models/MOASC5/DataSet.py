...
class DataSet:
    def __init__(self, dataset_base, dataset_name):
        self.dataset_name = dataset_name # 'MOA-MVSA-single' 'MOA-MVSA-multiple'
        self.dataset_dir = os.path.join(dataset_base, dataset_name)
        ...
        print(f'{self.texts_path}\n{self.images_path}')
        


        os.chdir(self.dataset_dir)
        print(f"[DEBUG] Now in dataset directory for labels: {os.getcwd()}")
        print(f"[DEBUG] About to read labels from: {os.path.abspath('labels.csv')}")
        
        print("Dataset directory is:", self.dataset_dir)
        print("Files in dataset directory:", os.listdir(self.dataset_dir))
