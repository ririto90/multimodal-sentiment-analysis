...
def overall_sentiment(row):  # which numeric values?
...
class DataSet:
    def __init__(self, dataset_name):
        self.dataset_name = dataset_name # 'MOA-MVSA-single' 'MOA-MVSA-multiple'
        self.dataset_dir = os.path.join('/home/rgg2706/Multimodal-Sentiment-Analysis/Datasets', self.dataset_name)
...
class MVSAAdapter(Dataset):
    """A small wrapper that:
       - Takes the arrays from your `DataSet` class
       - Tokenizes text with BERT
       - Returns a dict matching what the Instructor expects
    """
    def __init__(self, texts, images, labels, max_seq_len):
        self.texts = texts
        self.images = images          # e.g. a list or array of image tensors
        self.labels = labels          # sentiment labels
        self.max_seq_len = max_seq_len
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    ...
    def __getitem__(self, idx):
        text = self.texts[idx]
        img_tensor = self.images[idx]  # Should already be a torch.Tensor
        polarity = self.labels[idx]
