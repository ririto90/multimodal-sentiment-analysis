...
class MultiAttFusion(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # If you prefer to return raw logits (for CrossEntropyLoss), 
        # you can omit the softmax here. But if you want explicit softmax:
    ...
    def forward(self, text_features, image_features):
        ...
        #    - We want shape [B, 1, 768] for "k", so unsqueeze
        k = visual_reps.unsqueeze(1)       # [B, 1, 768]
        q = text_features                  # [B, seq_len, 768]

        # The Attention class returns shape [B, seq_len, 768]
        attended_text = self.attn(k=k, q=q)  # cross-attention

        # 5) Summarize the attended text. We'll replicate the old logic:
        #    In the old code, you had "v_alpha" => shape [B, 768], then multiplied by cls.
        #    So let's just do a mean across tokens => [B, 768].
        #    Or we can replicate "attn = cls_rep * v_alpha" exactly, see below:
        attended_text_mean = attended_text.mean(dim=1)  # [B, 768]

        # Multiply with the original CLS embedding, as old code does:
        attn_mult = cls_rep * attended_text_mean  # [B, 768]

        # 6) Concat that with the projected image
        multimodal_reps = torch.cat([visual_reps, attn_mult], dim=-1)  # [B, 768 + 768 = 1536]
