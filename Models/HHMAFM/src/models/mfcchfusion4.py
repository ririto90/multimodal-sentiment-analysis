...
class MFCCHFUSION4(nn.Module):
    def __init__(self, opt, text_dim, resnet_dim, densenet_dim):
        super(MFCCHFUSION4, self).__init__()
        self.opt = opt
        self.first_batch = True

        hidden_dim = opt.hidden_dim  # 512, 768, 1024
        ...
        fusion_dim = text_dim + 2 * hidden_dim
    ...
    def forward(self, roberta_text_features, roberta_topic_features, resnet_features, densenet_features, _):
        ...
        # Add original features to self-attended features
        text_self_attended = text_self_attended + roberta_text_features
        topic_self_attended = topic_self_attended + roberta_topic_features
        resnet_self_attended = resnet_self_attended + resnet_features
        densenet_self_attended = densenet_self_attended + densenet_features
        ...
        # Add original projected features to cross-attended features
        text_resnet_attended = text_resnet_attended + text_proj
        ...
        # Add original projected features to cross-attended features
        topic_densenet_attended = topic_densenet_attended + topic_proj
        ...
        # Add original projected features to co-attended features
        co_attended_text_resnet = co_attended_text_resnet + text_proj + resnet_proj
        ...
        # Add original projected features to co-attended features
        co_attended_topic_densenet = co_attended_topic_densenet + topic_proj + densenet_proj
        
        ### Compute mult_1 and mult_2 ###
        mult_1 = text_resnet_attended + co_attended_text_resnet
        mult_2 = topic_densenet_attended + co_attended_topic_densenet

        # Concatenate the outputs
        fusion = torch.cat([mult_1, mult_2, roberta_text_features], dim=1)
