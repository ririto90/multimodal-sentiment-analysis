...
class DataSet:
    def __init__(self, current_working_directory):
        self.dataset_name = 'MVSA_Multiple2' # dataset name: MVSA_Single/MVSA_Multi
        ...
        self.images_path = join_path(self.dataset_dir, 'images')

        os.chdir(self.dataset_dir)
        
        labels = pd.read_csv('labels.tsv', sep='\t').dropna()
        labels = labels.sort_values(by = ['ID'])

        # Added
        labels[["text", "image"]] = labels["text,image"].str.split(",", expand=True)
        labels.drop(columns=["text,image"], inplace=True)
