    ...
    def train(train_loader, validation_loader, test_loader, params, device):
        ...
        for epoch in range(params.epochs):
            ...
            if acc_test > best_test_accuracy:
                ...
                torch.save(
                    model,
                    os.path.join("/Users/ronengold/Library/Mobile Documents/com~apple~CloudDocs/Repositories/MultimodalOpinionAnalysis-main/SavedModels",
                                 "model10_{:.0f}_{:.0f}".format(acc_test * 100, f1_pn * 100))
                )
            test_accuracy.append(acc_test)
            test_loss.append(loss_test)
            average_recall.append(avg_recall)
