...
for name in datasets_names:
    datasets_path.append(os.path.join(datasets_dir, name + '/'))
...
# create a directory for the processed files of the original MVSA dataset
    ...
    labels_MVSA_Single.to_csv(datasets_dir + 'MVSA_Single/labels.tsv', sep = '\t', index = False)
    ...
    labels_MVSA_Multi.drop(indexes, inplace=True)
    labels_MVSA_Multi.to_csv(datasets_dir + 'MVSA_Multi/labels.csv', sep = '\t', index = False)
