...
class Attention(nn.Module):
    ...
    def forward(self, k, q, memory_len):
        ...
        # print(f"memory_len shape: {memory_len.shape}")
        ...
        # Debugging prints for the mask and masked attentions
        # print(f"mask shape: {mask.shape}")
        # print(f"masked shape: {masked.shape}")
        
        #print(masked[:2])
        #print(masked.shape)
        _sums = masked.sum(dim=(-1, -2), keepdim=True)  # sums per row
        # attentions = torch.div(masked, _sums.view(_sums.size(0), 1))
        attentions = torch.div(masked, _sums)
        #print(attentions[:2])
        
        # Debugging prints for the sums and final attentions
        # print(f"_sums shape: {_sums.shape}")
        # print(f"final attentions shape: {attentions.shape}")
        
        # score = torch.unsqueeze(attentions, dim=1)
        score = attentions.mean(dim=1, keepdim=True)
